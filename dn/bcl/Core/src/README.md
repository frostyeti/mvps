...
FrostYeti Core extends the the .NET Base Class Library with key functionality such as:
...
```csharp
using static FrostYeti.IO.FileSystem;
...
var res = TryCatch(() => {
    FrostYeti.IO.FileSystem.Chown("/opt/app", "user", "user");
});
